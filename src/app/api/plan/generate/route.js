...
function normalizePlan(plan) {
  ...
  plan.tasks = plan.tasks.map((t, i) => {
    ...
    if (!timelineSet.has(deadline)) {
      ...
      else if (plan.timeline.length > 0) deadline = plan.timeline[0].time;
    }
...
export async function POST(req) {
  const now = Date.now();
  console.log('[/api/plan/generate] HIT:', now);
  ...
  try {
    ...
    try {
      ...
      console.log("[/api/plan/generate] REAL SUCCESS! UUID:", planId, "Token:", shareToken);
